...
// MuscleTargeting is the output of the muscle targeting generation node.
// Decides primary/secondary muscle emphasis and distribution before strategy selection.
// If explicit muscles are provided by user, they are respected; otherwise inferred from goals/history.
type MuscleTargeting struct {
    Summarizable
    Primary      []string           `json:"primary"`               // high-volume emphasis (1-4 muscles)
    Secondary    []string           `json:"secondary"`             // maintenance (0-4 muscles)
    Distribution map[string]float64 `json:"distribution,omitempty"` // muscle -> fraction 0.0-1.0, sum ~ 1.0
    Reasoning    string             `json:"reasoning"`             // one sentence why these muscles
}
