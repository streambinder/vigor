...
func TestValidate_WeightModifierConsistency(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := tr.Validate(validExercises, nil, validModifiers, validRoutines, weightedModifiers, nil, false)
...
func TestValidate_DurationTolerance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tr := makeTestTraining()
			if err := tr.Validate(validExercises, nil, validModifiers, validRoutines, weightedModifiers, nil, false); err != nil {
...
func TestPurgeRepsDuration(t *testing.T) {
	tests := []struct {
		...
		{"duration-based: both set, keep duration", true, 30, 15, 30, 0},
		{"duration-based: both set, keep duration (2)", true, 90, 8, 90, 0},
		{"reps-based: both set, keep reps", false, 30, 10, 0, 10},
		{"reps-based: both set, keep reps (2)", false, 40, 12, 0, 12},
		{"duration-based: only duration, no change", true, 30, 0, 30, 0},
		{"reps-based: only reps, no change", false, 0, 10, 0, 10},
		{"duration-based: only reps, no change", true, 0, 15, 0, 15},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tr := makeTraining("strength", []Routine{
			...
			tr.PurgeRepsDuration(tt.durationBased)
